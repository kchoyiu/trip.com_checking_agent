...
The Compose file defaults to 台北, 2026-10-07 through 2026-10-11, and checks once every hour. Change `HOTEL_CITY`, `HOTEL_CHECK_IN`, and `HOTEL_CHECK_OUT` in `.env` to change the search without editing the Compose file. If Trip.com does not expose a usable suggestion, set the matching `HOTEL_CITY_ID` explicitly (Taipei is `617`). For a city not built into the scraper, also set its Trip.com slug in `HOTEL_CITY_SLUG`; the scraper then uses Trip.com's normal city-page search form. CAPTCHA/anti-bot pages still stop the run; Docker does not bypass them.
