...
from .engine.airlines import airline_matches, configured_airlines
from .engine.deals import evaluate
from .notification.telegram import TelegramNotifier, format_deal
from .scraper.trip import PageRouteMismatch, TripScraper, BotDetected
...
def _run_once():
    ...
    active_rows = [db.upsert_job(job) for job in generate_jobs(cfg)]
    active_ids = [row["id"] for row in active_rows if row]
    rows=db.due_jobs(int(settings.queue.get("max_jobs_per_run",8)), active_ids)
    preferred_airlines = configured_airlines(cfg)
    ...
    for row in rows:
        try:
            ...
            if preferred_airlines:
                prices = [price for price in prices if airline_matches(price.airline, preferred_airlines)]
                if not prices:
                    log.warning("No matching airline result for %s → %s; filter=%s", row["origin"], row["destination"], ", ".join(preferred_airlines))
        ...
        except PageRouteMismatch as e:
            logging.warning(str(e)); db.mark_checked(row["id"])
